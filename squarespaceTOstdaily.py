...
def parse_sheet(csv_text, address_col="Address", message_col="Message optional"):
    ...
    for row in reader:
        ...
        people.append({"first": first, "last": last, "email": email,
                        "address": address, "message": message, "phone": phone,
                        "tags": tags, "submitted_on": (row.get("Submitted On") or "").strip()})
    return people
...
def main():
    ...
    if st.button("Sync Now", type="primary"):
        ...
        # Store for message browsing
        st.session_state["synced_people"] = people

    # Messages section (persists after sync)
    if "synced_people" in st.session_state:
        people = st.session_state["synced_people"]
